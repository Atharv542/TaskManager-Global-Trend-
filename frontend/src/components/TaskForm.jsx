  ...
  return(
    <div className="p-6 flex justify-center py-50 ">
      <form onSubmit={submit} className="bg-slate-900 p-6 rounded w-full max-w-md py-10">
        ...
        <input className="input border-2 border-white rounded-md mt-5" placeholder="Enter title" onChange={e=>setTitle(e.target.value)} value={title} required/>
        <textarea className="input resize-none border-2 rounded-md mt-3 border-white" placeholder="Enter description" onChange={e=>setDescription(e.target.value)} value={description} required/>
